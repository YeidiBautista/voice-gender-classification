...
for i = 1:length(files)
    ...
    % No trim because record only 1,2s, trim make it too short for pitch,
    % create a problem that cannot fix
...
numCoeffs = 13;
    ...
    mfMean=median(mf,1); % 1 x numberCoeffs
...
model = fitcsvm(XtrainData, YtrainLabels,'KernelFunction','rbf','Standardize',true,'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('UseParallel', true,'ShowPlots',false));
...
for j = 1:length(k_values)
    k_i = k_values(j);
    Mdl_i = fitcknn(XtrainData, YtrainLabels, 'NumNeighbors', k_i);
    cvmdl = crossval(Mdl_i,'KFold',5);
    losses(j)= kfoldLoss(cvmdl);
end
[~,bestIdx]=min(losses);
best_k = k_values(bestIdx);
Mdl = fitcknn(XtrainData, YtrainLabels, "NumNeighbors",best_k, 'OptimizeHyperparameters','auto');
